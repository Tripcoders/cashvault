      ...
      <div className="relative w-full max-w-lg bg-background border border-border shadow-2xl rounded-2xl overflow-hidden animate-scale-in-up glass-strong">
              ...
              <div key={s.num} className="flex flex-col items-center gap-2 bg-muted/50 px-2">
                ...
                <span
                  className={`text-xs font-bold tracking-wide transition-colors ${
                    step >= s.num ? 'text-blue-600' : 'text-muted-foreground'
                  }`}
                >
                  {s.label}
                </span>
        ...
        <div className="p-8">
          {step === 1 && (
            <div className="space-y-6 animate-face-in-up">
              <div className="space-y-3">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-5 bg-background border-2 border-border focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 rounded-xl outline-none transition-all font-mono text-3xl font-bold text-foreground hover:border-blue-400"
                    placeholder="0.00"
                    autoFocus
                  />
                </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {['50', '100', '500'].map((val, idx) => (
                  <button
                    key={val}
                    onClick={() => setAmount(val)}
                    className={`py-4 px-4 text-sm font-bold text-muted-foreground bg-muted/50 border-2 border-border rounded-xl hover:border-blue-600 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all hover-lift ${
                      amount === val ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20 text-blue-600' : ''
                    }`}
                  >
                    ${val}
                  </button>
              ...
              {/* Security Notice */}
              <div className="flex items-center gap-3 p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-200 dark:border-blue-900/30 animate-fade-in-up stagger-1">
          ...
          {step === 2 && (
            <div className="space-y-6 animate-face-in-up">
              {/* Crypto Selection */}
              <div className="flex gap-3 p-1.5 bg-muted rounded-xl animate-fade-in-up">
                {['BTC', 'ETH', 'USDT'].map((crypto, idx) => (
                  <button
                    key={crypto}
                    onClick={() => setSelectedCrypto(crypto)}
                    className={`flex-1 py-3 rounded-lg text-sm font-bold transition-all hover-lift ${
              ...
              <div className="p-5 bg-blue-50/50 dark:bg-blue-900/10 border-2 border-blue-200 dark:border-blue-900/30 rounded-xl animate-fade-in-up stagger-1 hover:border-blue-300 dark:hover:border-blue-800/40 transition-colors">
              ...
              <div className="space-y-3 animate-fade-in-up stagger-2">
                ...
                <div className="flex items-center gap-3 p-4 bg-muted/50 border-2 border-border rounded-xl group hover:border-blue-500/30 transition-all hover-lift">
              ...
              <div className="flex items-center justify-center py-6 animate-fade-in-up stagger-3">
                <div className="w-44 h-44 bg-background p-4 rounded-xl border-2 border-border shadow-lg hover:shadow-xl hover:border-blue-500/30 transition-shadow">
                  {/* Mock QR Code */}
